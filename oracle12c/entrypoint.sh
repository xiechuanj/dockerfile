	...
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata)" ]; then
			...
			ln -s /u01/app/oracle/dbs /u01/app/oracle-product/12.1.0/xe/dbs
                        #get ipaddress
                        ips=$(ifconfig|sed -n '/inet addr/s/^[^:]*:\([0-9.]\{7,15\}\) .*/\1/p')
                        for ip in $ips
                        do
                            echo $ip | grep "127.0.0.1" #> /dev/null 2>&1 
                            if [ $? != 0 ]; then
                        	ipaddress=$ip
                            fi
                        done                       
                        sed 's/localhost/'${ipaddress}'/' /listener.ora.bak >/u01/app/oracle/product/12.1.0/xe/network/admin/listener.ora
                        sed 's/localhost/'${ipaddress}'/' /tnsnames.ora.bak >/u01/app/oracle/product/12.1.0/xe/network/admin/tnsnames.ora
                        #Startup Database
			...
			ln -s /u01/app/oracle/dbs /u01/app/oracle-product/12.1.0/xe/dbs
                        echo "get ipaddress"
                        ip=$(ifconfig|sed -n '/inet addr/s/^[^:]*:\([0-9.]\{7,15\}\) .*/\1/p'|grep '172.')
#                        echo "1111111111111"
#                        echo $ip
          #              for ip in $ips
          #              do
          ...
          #              echo $ipaddress
 #                       ls /
                        sed 's/localhost/'$ip'/' /listener.ora.bak >/u01/app/oracle/product/12.1.0/xe/network/admin/listener.ora
                        sed 's/localhost/'$ip'/' /tnsnames.ora.bak >/u01/app/oracle/product/12.1.0/xe/network/admin/tnsnames.ora
